...
$(document).ready(function () {
    ...
    function init() {
        $('.photo-list .photo-entry').remove();
        $('#no-new-photo').hide();
        $('#home-no-invi').hide();
        $('#home-no-message').hide();
        $('.home-message-list .home-message-entry').remove();
        $('.home-message-list .home-invitation-entry').remove();
    ...
    function getProfile(userID) {
        var url = "/api/profile?to_see_user_id=" + userID;
        var profile = {};
        $.get({
            url: url,
            dataType: 'json',
    ...
    function loadHome() {
            ...
            success: function(data) {
                ...
                if (invis.length > 0) {
                    $.each(invis, function(i, invitation) {
                        createInvitationEntryInHome(invitation);
                    });
                } else {
                    $('#home-no-invi').show();
                }
        ...
        //load friends then load messages with each friend
        $.get({
            ...
            success: function(data) {
                ...
                let messages = [];
                let friendTrack = 0;
                let friendPhotoTrack = 0;
                let photos = []
                $.each(friends, function(i, friend) {
                    $.get({
                        url: document.location.origin + "/api/message?friend_user_id=" + friend.user_id + "&load_rows=" + 10 + "&offset=" + 0,
                        success: function (data) {
                            friendTrack++;
                            $.each(data.messages, function(i, msg) {
                                if (msg.to_from === 'from') {
                                    msg.from_user_id = friend.user_id;
                                    messages.push(msg);
                ...
                const tracker = window.setInterval(function() {
                    if (friendTrack === friends.length) {
                        if (messages.length > 0) {
                            messages.sort(function(a, b) {
                                const t1 = new Date(a.time_stamp);
                                const t2 = new Date(b.time_stamp);
                                return t1 < t2? 1 : -1;
                            });
                            $.each(messages, function(i, message) {
                                loadMessagesInHome(message);
                            });
                        } else {
                            $('#home-no-message').show();
                        }
                ...
                $.each(friends, function(i, friend) {
                    $.get({
                        url: document.location.origin + "/api/photo?friend_user_id=" + friend.user_id + "&load_rows=" + 10 + "&offset=" + 0,
                        success: function(data) {
                            friendPhotoTrack++;
                ...
                const photoTracker = window.setInterval(function() {
                    if (friendPhotoTrack === friends.length) {
                        if (photos.length > 0) {
                            photos.sort(function(a, b) {
                                const t1 = new Date(a.time_uploaded);
                                const t2 = new Date(b.time_uploaded);
                                return t1 < t2? 1 : -1;
                            });
    ...
    function loadMessagesInHome(message) {
        const tstamp = formatDate(new Date(message.time_stamp));
        const entry = $('<div/>', {
            'class': 'home-message-entry list-group-item',
        });
        $.get({
            url: document.location.origin + "/api/profile?to_see_user_id=" + message.from_user_id,
            success: function (data) {
                const profile = data;
                ...
                entry.append($('<a/>', {
                    ...
                    .append($('<h4/>', {'class': 'from-who text-center'}).text(name))
                    .append($('<div/>', {'class': 'text-content'}).text(message.content))
                    .append($('<p/>', {'class': 'text-right'}).text(tstamp))
                    .appendTo('#home-msg-list');
            }
        });

    }

    function createInvitationEntryInHome(invitation) {
        const entry = $('<div/>', {
        ...
        $.get({
            ...
            success: function(data) {
                ...
                entry.append($('<a/>', {
                    ...
                    .append(acceptBtn)
                    .append(declineBtn).appendTo('#home-invitation-list');
                acceptBtn.click(function() {
                    $.post({
                        ...
                        url: document.location.origin + "/api/invitation",
                        success: function () {
                            $(entry).text(name + "\'s invitation accepted!");
                        }
                    });
                });
                declineBtn.click(function() {
                    $.post({
                        ...
                        url: document.location.origin + "/api/invitation",
                        success: function () {
                            $(entry).text(name + "\'s invitation declined!");
                        }
                    });
                });
            }
