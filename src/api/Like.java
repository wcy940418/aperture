...
public class Like extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{
            ...
            JSONObject request = RpcParser.parseInput(req);
            RpcParser.checkKeys(request, "photo_id");
            JSONArray likers = conn.getPhotoLikeList(request.getInt("user_id"), request.getInt("photo_id"));
            response.put("likers", likers);
            response.put("photo_id", request.getInt("photo_id"));
        } catch (Exception e) {
            e.printStackTrace();
            resp.setStatus(400);
            response.put("status", e.getMessage());
